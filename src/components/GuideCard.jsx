    ...
    >
        ...
        {pct > 0 && (
          <>
            <div className="absolute bottom-0 inset-x-0 h-1 bg-black/30">
              <div
                className={`h-full transition-all duration-500 ${pct === 100 ? 'bg-emerald-400' : 'bg-amber-500'}`}
                style={{ width: `${pct}%` }}
              />
            </div>
            {pct === 100 && (
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-center gap-2 mb-2.5">
          ...
          <span className={`text-[11px] flex items-center gap-1 ${dark ? 'text-stone-500' : 'text-stone-400'}`}>
            <Clock size={11} /> {guide.estimatedTime}
          </span>
          {pct > 0 && pct < 100 && (
            <span className="ml-auto text-[11px] font-semibold text-amber-600">{pct}%</span>
          )}
        </div>
        ...
        <p className={`text-[12px] leading-relaxed line-clamp-2 flex-1 ${dark ? 'text-stone-500' : 'text-stone-500'}`}>
          {guide.summary}
        </p>

        <div className={`flex items-center justify-between mt-3 pt-3 border-t ${dark ? 'border-stone-800' : 'border-stone-100'}`}>
          <div className="flex items-center gap-2">
            <img src={guide.author.avatar} alt="" className="w-5 h-5 rounded-full object-cover" />
            <span className={`text-[11px] font-medium ${dark ? 'text-stone-500' : 'text-stone-400'}`}>
              {guide.author.name}
            </span>
          </div>
