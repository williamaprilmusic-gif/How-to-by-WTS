...
export default function GeneratePanel({ onGuideGenerated, existingGuides, dark }) {
  ...
  return (
    <div className={`rounded-xl border mb-8 overflow-hidden ${dark ? 'bg-stone-900 border-stone-800' : 'bg-white border-stone-200'}`}>
      {/* Header */}
      <div className={`px-5 py-4 border-b flex items-center gap-3 ${dark ? 'border-stone-800' : 'border-stone-100'}`}>
      ...
      {ALL_CATEGORIES.map(cat => {
        ...
        return (
          <div key={cat} className={`border-b last:border-0 ${dark ? 'border-stone-800' : 'border-stone-100'}`}>
            {/* Category header */}
            <button
              onClick={() => setExpanded(isExpanded ? null : cat)}
              className={`w-full flex items-center gap-3 px-5 py-3 text-left transition-colors ${dark ? 'hover:bg-stone-800/60' : 'hover:bg-stone-50'}`}
            >
              <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: meta.color }} />
              <span className={`text-[13px] font-medium flex-1 ${dark ? 'text-stone-300' : 'text-stone-700'}`}>{cat}</span>
              <span className={`text-[11px] font-semibold ${catDone === topics.length ? 'text-emerald-500' : dark ? 'text-stone-500' : 'text-stone-400'}`}>
                {catDone}/{topics.length}
              </span>

              {isExpanded ? <ChevronUp size={14} className={dark ? 'text-stone-500' : 'text-stone-400'} /> : <ChevronDown size={14} className={dark ? 'text-stone-500' : 'text-stone-400'} />}
            </button>

            {/* Topic list */}
            {isExpanded && (
              <div className={`px-5 pb-3 grid grid-cols-1 sm:grid-cols-2 gap-1.5 ${dark ? 'bg-stone-900/50' : 'bg-stone-50/50'}`}>
                {topics.map(topic => {
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-left transition-colors text-[12px] ${
                        s === 'done' || isExisting
                          ...
                          : s === 'loading'
                          ? dark ? 'bg-amber-900/20 text-amber-500' : 'bg-amber-50 text-amber-600'
                          : dark ? 'hover:bg-stone-700 text-stone-400' : 'hover:bg-white text-stone-600'
                      }`}
                    >
